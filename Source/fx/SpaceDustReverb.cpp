...
void SpaceDustReverb::prepare(const juce::dsp::ProcessSpec& spec)
{
    ...
    dryScratch_.setSize(2, juce::jmax(1, (int) spec.maximumBlockSize), false, true, true);
}
...
void SpaceDustReverb::process(juce::AudioBuffer<float>& buffer)
{
    ...
    // Sol port: pre-allocated dry copy (see dryScratch_ in the header). The
    // defensive grow only fires if a host exceeds the prepared block size.
    if (dryScratch_.getNumSamples() < numSamples)
        dryScratch_.setSize(2, numSamples, false, true, true);
