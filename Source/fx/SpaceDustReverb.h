...
private:
    ...
    // Sol port: the dry copy is a member sized in prepare(). Upstream builds a
    // local AudioBuffer inside process(), which allocates on the audio thread —
    // Sol's chain forbids that (see EffectChain.h).
